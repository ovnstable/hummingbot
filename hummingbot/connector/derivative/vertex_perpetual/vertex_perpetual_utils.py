...
def trading_pair_to_product_id(trading_pair: str, exchange_market_info: Dict, is_perp: Optional[bool] = False) -> int:
    ...
    for product_id in exchange_market_info:
        if is_perp and "perp" not in exchange_market_info[product_id]["symbol"].lower():
            continue
        if exchange_market_info[product_id]["market"].replace("-PERP", "") == tp:
            return product_id
    return -1
...
def perp_to_spot_product_id(trading_pair: str, exchange_market_info: Dict) -> int:
    base = trading_pair.split('-')[0]
    for product_id in exchange_market_info:
        if exchange_market_info[product_id]["symbol"].split('/')[0] == base:
            return product_id
    return -1
...
def generate_expiration(timestamp: float, order_type: Optional[str] = None) -> str:
    ...
    expiration = str((unix_epoch + 86400) | (sig_bit << 62))
    return expiration


# TODO: Create note for this where if you have errors with expired time, look at this..
...
def convert_address_to_sender(address: str) -> str:
    if isinstance(address, str):
        # TODO: The default subaccount is called default... Not sure how this can be used otherwise, but review it's implementation.
...
class VertexPerpetualConfigMap(BaseConnectorConfigMap):
    connector: str = Field(default="vertex_perpetual", const=True, client_data=None)
    vertex_perpetual_arbitrum_private_key: SecretStr = Field(
        default=...,
    ...
    vertex_perpetual_arbitrum_address: str = Field(
        default=...,
        client_data=ClientFieldData(
    ...
    class Config:
        title = "vertex_perpetual"


KEYS = VertexPerpetualConfigMap.construct()


class VertexPerpetualTestnetConfigMap(BaseConnectorConfigMap):
    connector: str = Field(default="vertex_perpetual_testnet", client_data=None)
    vertex_perpetual_testnet_arbitrum_private_key: SecretStr = Field(
        default=...,
        client_data=ClientFieldData(
    ...
    vertex_perpetual_testnet_arbitrum_address: str = Field(
        default=...,
        client_data=ClientFieldData(
    ...
    class Config:
        title = "vertex_perpetual_testnet"


OTHER_DOMAINS = ["vertex_perpetual_testnet"]
OTHER_DOMAINS_PARAMETER = {"vertex_perpetual_testnet": "vertex_perpetual_testnet"}
OTHER_DOMAINS_EXAMPLE_PAIR = {"vertex_perpetual_testnet": "WBTC-USDC"}
OTHER_DOMAINS_DEFAULT_FEES = {"vertex_perpetual_testnet": DEFAULT_FEES}

OTHER_DOMAINS_KEYS = {"vertex_perpetual_testnet": VertexPerpetualTestnetConfigMap.construct()}
