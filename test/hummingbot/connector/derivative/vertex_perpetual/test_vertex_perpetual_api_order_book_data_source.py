...
from hummingbot.connector.derivative.vertex_perpetual import vertex_perpetual_constants as CONSTANTS
...
class TestVertexPerpetualAPIOrderBookDataSource(unittest.TestCase):
    ...
    def setUp(self) -> None:
        ...
        self.ob_data_source = VertexPerpetualAPIOrderBookDataSource(
            trading_pairs=[self.trading_pair],
        ...
        exchange_rules = {
            ...
            "data": {
                "perp_products": [
                    {
                        "product_id": 2,
                        "oracle_price_x18": "26440071280208533000000",
                        "risk": {
                        ...
                        "state": {
                            "cumulative_funding_long_x18": "135046746334883405543602",
                            "cumulative_funding_short_x18": "135046746334883405543602",
                            "available_settle": "0",
                            "open_interest": "41459000000000000000"
                        },
                        "lp_state": {
                            "supply": "264410712802085330000000",
                            "last_cumulative_funding_x18": "135046746334883405543602",
                            "cumulative_funding_per_lp_x18": "0",
                            "base": "10000000000000000000",
                            "quote": "264400712802085330000000"
                        },
                        "book_info": {
                            "size_increment": "1000000000000000",
                            "price_increment_x18": "1000000000000000000",
                            "min_size": "10000000000000000",
                            "collected_fees": "166772422284456891359",
    ...
    def _snapshot_response() -> Dict:
        snapshot = {
            ...
            "data": {
                "bids": [
                    ["26000000000000000000000", "1000000000000000000"],
                    ["25000000000000000000000", "2000000000000000000"],
                ],
                "asks": [
                    ["26000000000000000000000", "3000000000000000000"],
                    ["27000000000000000000000", "4000000000000000000"],
                ],
    ...
    def test_request_order_book_snapshot(self, mock_api):
        url = f"{CONSTANTS.BASE_URLS[self.domain]}/query?depth={CONSTANTS.ORDER_BOOK_DEPTH}&product_id=2&type={CONSTANTS.MARKET_LIQUIDITY_REQUEST_TYPE}"
    ...
    def test_get_new_order_book(self, mock_api):
        ...
        self.assertEqual(26000, bid_entries[0].price)
        ...
        self.assertEqual(27000, ask_entries[1].price)
    ...
    def test_listen_for_subscriptions_subscribes_to_trades_and_depth(self, ws_connect_mock):
        ...
        result_subscribe_trades = {"method": "subscribe", "stream": {"type": "trade", "product_id": 2}, "id": 2}

        result_subscribe_depth = {"method": "subscribe", "stream": {"type": "book_depth", "product_id": 2}, "id": 2}
        ...
        expected_trade_subscription = {"method": "subscribe", "stream": {"type": "trade", "product_id": 2}, "id": 2}
        self.assertEqual(expected_trade_subscription, sent_subscription_messages[0])
        expected_diff_subscription = {"method": "subscribe", "stream": {"type": "book_depth", "product_id": 2}, "id": 2}
